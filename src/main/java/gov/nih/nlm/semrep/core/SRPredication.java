 ...
 *
 */
public class SRPredication extends AbstractRelation implements HasPredicate {
	private static Logger log = Logger.getLogger(SRPredication.class.getName());
...
//	private boolean resolved = false;
	...
	public SRPredication(String id) {
		super(id);
	}
	 ...
	 */
	public SRPredication(String id, Predicate predicate, List<Argument> args) {
		super(id,SRPredication.GENERIC);
...
//		if (span != null) return span;
		...
		for (int i= 0; i < arguments.size(); i++) {
			...
			if (si instanceof SRPredication) argsp = ((SRPredication)si).getSpan();
...
//		buf.append(",");
		...
		buf.append(")");
		// assuming Subject and Object arguments
		List<Argument> subjects = getArgs("Subject");
		List<Argument> objects = getArgs("Object");
		Entity subj = (Entity) getSubject();
		Entity obj = (Entity) getObject();
		if (objects != null || objects.size()> 0) obj = (Entity)objects.get(0).getArg();
		for (int i=0; i < arguments.size() -1; i++) {
			Argument arg = arguments.get(i);
			buf.append(arg.toShortString() + "_");
		}
...
//		}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SRPredication other = (SRPredication) obj;
		if (id == null) {
	 ...
	 */
	public static boolean outscopes(SRPredication pred1, SRPredication pred2) {
		...
		for (Argument a1: srargs1) {
			SemanticItem si = a1.getArg();
			if (si instanceof SRPredication) 
				if (outscopes((SRPredication)si,pred2) || 
						   sharedArg((SRPredication)si,pred2)) return true;
		}
		return false;
	}
	 ...
	 */
	public static boolean sharedArg(SRPredication pred1, SRPredication pred2) {
