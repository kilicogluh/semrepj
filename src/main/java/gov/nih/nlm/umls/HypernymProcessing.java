...
public class HypernymProcessing {
	...
	public List<Argument> intraNP(Chunk chunk) {
		...
		for(int i = size - 1; i >= 0; i--) {
			if (first == null) {
				if (((AbstractSurfaceElement) seList.get(i)).getChunkRole() == 'H') {
					first = seList.get(i);
					result = findInterNPwithinDistance(((AbstractSurfaceElement) seList.get(i)).getChunk(), (AbstractSurfaceElement)first);
					if (result != null) {
						return result;
					}
				}
	...
	public List<Argument> interNP(Chunk chunk) {
		...
		for(int i = size - 1; i >= 0; i--) {
				...
				if(((AbstractSurfaceElement) seList.get(i)).getChunkRole() == 'M') {
					second = seList.get(i);
	...
	public List<Argument> findInterNPwithinDistance(Chunk NP, AbstractSurfaceElement se) {
		if (NP == null) return null;

		ScoredUMLSConcept headConcept = null;
		String headCUI;

		SemanticItem headSem = getEntityFromSurfaceElement(se);
		...
		SRSentence srSentence = (SRSentence) se.getSentence();
		//look to the right of NP for hypernymy pair
		List<Chunk> chunks = srSentence.getChunks();
		Chunk NP2;
			...
			for (Chunk chunk: chunks) {
				if (chunk.getChunkType().equalsIgnoreCase("NP")) {//found NP pair
					...
					if (output != null && allowedGEOA(headConcept, head2Concept)) {//A potential ISA relationship is returned
						if (interveningPhrasesOK(NP, NP2)) {
							return output;
						}
					}
				}
