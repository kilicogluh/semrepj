...
bool TaggerImpl::open(const char* model) {
  close();


  feature_index_ = new DecoderFeatureIndex();
  CHECK_CLOSE_FALSE(feature_index_->open(model, 0))
      << feature_index_->what();

  double c = 1.0; // default cost value
...
const char* TaggerImpl::parse(const char* input, size_t length) {
  // printf("Inside parse(a,b):\n");
  std::istringstream is(std::string(input, length));
  // printf("Inside parse(a,b): step 2\n");
